...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        logger.info(torch.norm(density_fn(sample_fn(torch.Tensor(p_samples))) - torch.Tensor(p_samples)).item() / p_samples.shape[0])
        ...
        with h5py.File('8gaussGenFFJORD.h5', 'w') as f:
            f.create_dataset('x',       data=p_samples.detach().numpy())
            f.create_dataset('fx',      data=forw.detach().numpy())
            f.create_dataset('finvfx',  data=invForw.detach().numpy())
            f.create_dataset('y',       data=norm_samples.detach().numpy())
            f.create_dataset('finvy',   data=genSamples.detach().numpy())

        plt.savefig("../image/eval8gaussiansRK4.pdf")
